defmodule VivvoWeb.CoreComponents do
  ...
  defp custom_input(%{type: "select"} = assigns) do
    ...
    <select
      id={@id}
      name={@name}
      class={[@class, "w-full", @errors != [] && "select-error"]}
      {@rest}
    >
  ...
  defp custom_input(%{type: type} = assigns) do
    assigns =
      assigns
      |> assign(
        :error_class,
        if(type == "textarea", do: "textarea-error", else: "input-error")
      )
  ...
  attr :id, :string, required: true, doc: "the modal's id"
  slot :inner_block, required: true, doc: "the modal's content"

  def modal(assigns) do
    ~H"""
    <dialog id={@id} class="modal">
      <div class="modal-box relative p-8">
        {render_slot(@inner_block)}

        <form method="dialog">
          <!-- Top right close button -->
          <button class="btn btn-circle btn-sm btn-ghost absolute top-2 right-2">
            <.icon name="hero-x-mark" />
          </button>
        </form>
      </div>
